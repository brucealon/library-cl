...
select p.title as book_title,
       ...
       cast(pse.series_number as integer) as series_number
  from publications p
    ...
    join publication_series_entries pse on p.publication_id = pse.publication_id
    join publication_series ps on pse.publication_series_id = ps.publication_series_id
  where c.last_name = 'Crais'
  order by series_title, series_number
